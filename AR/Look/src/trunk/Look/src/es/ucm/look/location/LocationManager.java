...
public class LocationManager {
		...
		if (ins) {
			...
			TimerTask timerTask = new TimerTask() {
				public void run() {
					...
					location.set(location.x + mapCoords[0], location.y
							+ mapCoords[1], location.z);
	...
	/**
	 * Returns whether the user is walking or not @ return whether the user is
	 * walking
	 */
	...
	/**
	 * Returns the current wifi node information @ return current wifi node
	 * information
	 */
	...
	/**
	 * Returns the current position @ return the current position in (x,y,z)
	 * coordinates
	 */
	...
	public int getNodeId() {
		if (wifi)
			return wifiLocationProvider.getIdActual();
		else
			return 0;
	}

	public Point3 getDisplacement() {
		return new Point3(lastLocation.x - location.x, lastLocation.y
				- location.y, lastLocation.z - location.z);
	}
