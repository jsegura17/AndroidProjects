...
public class LookService extends Service {
	...
	private final IRemoteService.Stub binder = new IRemoteService.Stub() {
			...
			if (id != -1) {
				...
				if (response != null) {
					...
					if (responseString.equalsIgnoreCase("")) {
						...
						EntityData adata = new EntityData();
						adata.setId(id);
						
						//update the DB
						LookContentProvider.getInstance().updatePosition(adata, x, y, z);
		...
		public void addElement(String type, float x, float y, float z,
			...
			if (response != null) {
				...
				if (id != -1) {
					...
					entityData.setId(id);
					entityData.setLocation(x, y, z);
					
					LookContentProvider.getInstance().addEntity(entityData);
		...
		public void updateOrAddProperty(int id, String propertyName,
			...
			//First try to insert the property
			HttpResponse response = RestMethod.doPost(
					ConfigNet.getInstance().getURL("propertiess/"), objProp);
		...
		/*	String query = "SELECT e FROM Main e WHERE (e.x BETWEEN " + x1
					...
					+ ") AND (e.z = " + z + ")";
			*/
			String query = "SELECT e FROM Main e";
		...
		@Override
		public void getPropertiesUser(int id) throws RemoteException {

			Message msg = new Message();
			Bundle data = new Bundle();

			String userQuery = "SELECT e FROM Properties e WHERE e.propertiesPK.id = '"+ id + "'";
			...
			EntityData entityData =  new EntityData();
			try {
				recsProp = response.getJSONArray("properties");
				...
				// if no result
				Log.i("LookService", "User without properties");
			}
			catch (NullPointerException e) {
				// if no result
				Log.i("LookService", "Error with the user, it doesn´t exits");
			}

			LookData.getInstance().getLogger().setUser(entityData);
			
			data.putString("response", "1");

			msg.setData(data);

			msg.what = LookProperties.ACTION_GET_PROPERTIES_USER;
			mHandler.sendMessage(msg);
		}
			...
			for (int i = 0; i < N; i++) {
				try {
					...
					case LookProperties.ACTION_GET_PROPERTIES_USER:
						String response4 = msg.getData().getString("response");
						mCallbacks.getBroadcastItem(i).sendResponse4(response4);
						break;
					default:
