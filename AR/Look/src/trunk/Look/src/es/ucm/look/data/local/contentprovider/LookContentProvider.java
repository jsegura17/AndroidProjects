...
public class LookContentProvider implements DataHandler {
	...
	private synchronized static void createInstance() {
		if (INSTANCE == null) {
			INSTANCE = new LookContentProvider();
		}
	}
	...
	/**
	 * Add a new Entity, if it exits is updated.
	 */
	@Override
	public void addEntity(EntityData data) {
		...
		values.put(LookProperties.FIELD_ID, data.getId());
		...
		//values.put(LookProperties.FIELD_LAST_UPDATE, "datetime('now')");//TODO
		...
		try {
			result = c.insert(LookSQLContentProvider.MAIN_CONTENT_URI, values);
		}
		catch ( Exception e ){
			e.printStackTrace();
		}
